...
impl RenderTarget {
    pub async fn new(width: f64, height: f64, callback: impl FnOnce(&mut RenderTarget, &mut GraphicsState) + Send + 'static) -> TatakuResult<Self> {
    ...
    pub async fn update(&mut self, callback: impl FnOnce(&mut RenderTarget, &mut GraphicsState) + Send + 'static) {
        // let t = self.clone();
