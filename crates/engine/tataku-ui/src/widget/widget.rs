...
pub trait Widget<Action: Send + Sync>: Send + Sync {
    ...
    fn all_children(&self) -> WidgetChildren<Action> { self.children() }
    fn all_children_mut(&mut self) -> WidgetChildrenMut<Action> { self.children_mut() }

    /// helper for default actions
    fn children(&self) -> WidgetChildren<Action> { WidgetChildren::None }
    /// helper for default actions
