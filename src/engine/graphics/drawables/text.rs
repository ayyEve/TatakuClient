...
impl Text {
    ...
    pub fn measure_text(&self) -> Vector2 {
        Self::measure_text_internal(&self.fonts, self.font_size, &self.text, self.scale, 2.0) 
    }
    ...
    fn measure_text_internal(fonts: &Vec<Font>, font_size: f32, text: &String, _scale: Vector2, line_spacing: f32) -> Vector2 {
...
impl TatakuRenderable for Text {
    ...
    fn draw_with_transparency(&self, alpha: f32, _: f32, mut transform: Matrix, g: &mut GraphicsState) {
        ...
        let font_size = self.font_size * self.scale.y;
        let mut x = 0.0;
        let mut y = font_size;
        ...
        for (ch, color) in text {
            ...
            'find_font: for i in self.fonts.iter() {
                ...
                if i.has_char_loaded(ch, font_size) {
