...
impl Text {
    ...
    pub fn measure_text_raw(
        ...
        for ch in text.chars() {
            ...
            for i in fonts {
                let font = ActualFont::get_font(*i);

                let Some(data) = font.get_character(font_size, ch) else { continue };
    ...
    fn draw(
        ...
        for ch in self.text.chars() {
            ...
            'find_font: for i in self.fonts.iter() {
                ...
                if !font.has_char_loaded(ch, font_size, g) { 
                    continue 
                }
                font.draw_character_image(
                    font_size, 
                    ch, 
                    [&mut x, &mut y], 
                    scale,
