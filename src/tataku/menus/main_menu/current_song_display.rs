...
const OUTER_PADDING:f64 = 5.0;
const INNER_PADDING:f64 = 8.0;
...
impl CurrentSongDisplay {
    ...
    pub fn update(&mut self) {
        ...
        if self.current_song.update() {
            ...
            let text = Text::new(PRIMARY_COLOR, -999.0, Vector2::ZERO, 30, self.song_text.clone(), get_font());
            self.text_size = text.measure_text();
        }
    }

    pub fn draw(&self, list: &mut RenderableCollection) {
        let mut text = Text::new(PRIMARY_COLOR, -999.0, Vector2::ZERO, 30, self.song_text.clone(), get_font());
