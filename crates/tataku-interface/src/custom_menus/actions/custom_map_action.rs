...
impl FromLua for CustomMenuMapAction {
        ...
        match lua_value {
            LuaValue::Table(table) => {
                ...
                match &*id {
                    ...
                    "random" => Ok(Self::Random(table.get::<Option<bool>>("use_preview")?.unwrap_or(true))),
                    ...
                    "previous" | "prev" => {
                        ...
                        let if_none = match if_none.as_deref() {
                            ...
                            Some(other) => return Err(FromLuaConversionError { 
                                from: "String", 
                                to: "MapActionIfNone".to_owned(), 
                                message: Some(format!("Unknown MapActionIfNone value {other}")) 
                            })
                        };
                    ...
                    other => Err(FromLuaConversionError { 
                        from: "String", 
                        to: "CustomMenuMapAction".to_owned(), 
                        message: Some(format!("Unknown map action {other}")) 
                    })
                }
            }
            LuaValue::String(action_str) => {
