    ...
    fn add_note(&mut self, mut note: MeasureEvent, mut time_step: f32) {
        ...
        if let Some(branch) = &mut self.current_branch {
            match note {
                ...
                MeasureEvent::LongEnd(last_long) => self.get_last_long(last_long).ok_do_mut(|l|l.set_end_time(time)),
            }
        } else {
            match note {
                MeasureEvent::EmptyNote => {}
    ...
    fn get_last_long(&mut self, long_type: LongType) -> Option<ExistingLongType> {
        match long_type {
            LongType::Balloon => {
                ...
                list.iter_mut().find(|b|b.end_time.is_nan()).map(|b|ExistingLongType::Balloon(b))
            }
            LongType::Drumroll => {
                let list = if let Some(b) = &mut self.current_branch {&mut b.drumrolls} else { &mut self.course.drumrolls };
                list.iter_mut().find(|d|d.end_time.is_nan()).map(|b|ExistingLongType::Drumroll(b))
            }
            LongType::None => None
        }
