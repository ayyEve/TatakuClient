...
impl FromLua for CustomMenuCursorAction {
        ...
        match lua_value {
            LuaValue::String(str) => {
                ...
                match &*str.to_str()? {
                    ...
                    other => Err(FromLuaConversionError { from: "String", to: THIS_TYPE.to_owned(), message: Some(format!("Invalid {THIS_TYPE} action: {other}")) }),
                }
            }

            other => Err(FromLuaConversionError { from: other.type_name(), to: THIS_TYPE.to_owned(), message: None })
        }
