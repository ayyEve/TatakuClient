...
impl ShuntingYard {
    ...
    fn run_function(
        ...
        // no-argument fns 
        #[allow(clippy::single_match, reason = "adding more later")]
        match function {
            "now" => {
                stack.push(Cow::Owned(TatakuValue::F32(TatakuInstant::now().as_millis())));
                return Ok(())
            }
...
impl Operator {
    ...
    fn perform<'a>(
        &self, 
        right: Cow<'a, TatakuValue>, 
        left: Cow<'a, TatakuValue>,
    ) -> Cow<'a, TatakuValue> {
        // debug!("");
