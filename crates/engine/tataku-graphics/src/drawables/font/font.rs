...
impl ActualFont {
    ...
    fn load_font_size(
        ...
        graphics.load_font_data(self.clone(), font_size);
    ...
    pub fn has_char_loaded(&self, ch: char, size: f32, graphics: &mut dyn GraphicsEngine) -> bool {
        ...
        if !loaded { self.load_font_size(font_size.f32(), graphics); }
    ...
    pub fn get_font(f: Font) -> &'static Self {
        match f {
            Font::Main => &MAIN_FONT,
            Font::Fallback => &FALLBACK_FONT,
            Font::FontAwesome => &FONT_AWESOME,
        }
    }
}
