    ...
    pub fn resolve(
        ...
        match self {
            Self::MenuAction(action) => {
                ...
                // let message = MessageValue::CustomMenuAction(action, passed_in);
            ...
            Self::SetValue { key, value } => {
                ...
                Some(Message::new(owner, "", MessageValue::SetValue(key.clone(), val)))
                // Some(Message::new(owner, "", MessageValue::CustomMenuAction(action, passed_in)))
            }
            Self::CustomAction { tag, value } => {
                ...
                Some(Message::new(owner, tag, MessageValue::Value(val)))
            }
            Self::Conditional { cond, if_true, if_false } => {
            ...
            Self::Multiple(list) => {
                ...
                (!list.is_empty()).then(|| Message::new(owner, "", MessageValue::Multi(list)))
            }

        }
    }
}
impl FromLua for LuaAction {
    fn from_lua(lua_value: LuaValue, _lua: &Lua) -> LuaResult<Self> {
        #[cfg(feature="debug_custom_menus")] info!("Reading LuaAction");
        let LuaValue::Table(table) = lua_value else { 
            return Err(FromLuaConversionError { 
                from: lua_value.type_name(), 
                to: "LuaAction".to_owned(), 
                message: Some("Not a table".to_owned()) 
            }) 
        };

        if table.get::<Self>(1).is_ok() {
            #[cfg(feature="debug_custom_menus")] info!("Is list");
            let mut list = Vec::new();

            for i in 1.. {
                let Ok(item) = table.get::<Self>(i) else { break };
        ...
        match &*id {
            ...
            "set_value" => Ok(Self::MenuAction(CustomMenuAction::SetValue(
                table.get("key")?,
                CustomEventValueType::from_lua(&table)?
            ))),
            // Ok(Self::SetValue {
            //     key: table.get("key")?,
            ...
            "conditional" => Ok(Self::Conditional {
                ...
                if_false: table.get::<Option<LuaAction>>("if_false")?.map(Box::new)
            }),

            other => Err(FromLuaConversionError { 
                from: "table", 
                to: "LuaAction".to_owned(), 
                message: Some(format!("unknown id: {other}")) 
            }),
        }
    }
}
...
impl CustomEventValueType {
    ...
    pub fn from_lua(table: &LuaTable) -> LuaResult<Self> {
        if let Some(value) = table.get::<Option<TatakuValue>>("value")? {
            Ok(Self::Value(value))
        } else if let Some(var) = table.get::<Option<String>>("variable")? {
            Ok(Self::Variable(var))
        } else if table.get::<Option<bool>>("passed_in")? == Some(true) {
            Ok(Self::PassedIn)
        } else {
            Ok(Self::None)
...
impl FromLua for CustomEventValueType {
    fn from_lua(lua_value: LuaValue, _lua: &Lua) -> LuaResult<Self> {
        const THIS_TYPE:&str = "CustomEventValueType";
        #[cfg(feature="debug_custom_menus")] info!("Reading {THIS_TYPE}");
        match lua_value {
            LuaValue::Table(table) => Self::from_lua(&table),
            LuaValue::String(s) => Ok(Self::new_value(s.to_str()?.to_owned())),
            LuaValue::Number(n) => Ok(Self::new_value(n as f32)),
            LuaValue::Integer(n) => Ok(Self::new_value(n as u64)),

            other =>  Err(FromLuaConversionError { 
                from: other.type_name(), 
                to: THIS_TYPE.to_owned(), 
                message: Some("Bad type".to_string()) 
            })
        }
    }
}
